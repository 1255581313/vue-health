        ...
        >
          ...
          >
            ...
            >
              <div slot="desc">
                <div class="desc">
                  <van-tag v-for="(spec, index) in goods.specifications" :key="index" plain style="margin-right: 6px">
                    {{ spec }}
                  </van-tag>
                </div>
              </div>
...
@click.stop="confirmOrder(el.id)"
                >确认收货</van-button
              >
              <van-button v-if="el.handleOption.delete" size="small" @click.stop="delOrder(el.id)">删除订单</van-button>
              <van-button
...
<script>
import { orderList, orderDelete, orderConfirm, orderCancel, orderRefund } from '@/api/health/api'
