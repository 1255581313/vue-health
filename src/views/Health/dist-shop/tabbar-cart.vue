<template>
  <div class="tab-cart">
    <div v-show="goods.length" class="editor_head">
    ...
    <van-checkbox-group v-model="checkedGoods" class="card-goods" @change="toggle">
      <div v-for="(item, i) in goods" :key="i" class="card-goods__item">
        <van-checkbox :key="item.id" v-model="item.checked" :name="item.id"></van-checkbox>
    ...
    <van-submit-bar
      ...
      @submit="cartSubmit"
    >
      <van-checkbox v-model="checkedAll" style="padding: 0 10px;" @click="setCheckAll">全选</van-checkbox>
    </van-submit-bar>
  </div>
</template>
...
import { cartList, cartUpdate, cartChecked, cartDelete } from '@/api/health/api'
...
export default {
  ...
  methods: {
    ...
    deleteCart(o) {
      const productId = this.goods[o].productId
      this.$dialog
    ...
    toggle(index) {
      const addProductIds = []
      _.each(index, v => {
        const productId = _.find(this.goods, result => {
          return result.id === v
        }).productId
